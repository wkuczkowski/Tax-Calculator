(function () {
  ...
  const DOM = {
    ...
    ryczaltCheckboxes: document.querySelectorAll('.checkbox-group input[type="checkbox"]'),
    ryczaltMessage: document.getElementById("ryczalt-message"),
    jointTaxationRadios: document.querySelectorAll('input[name="jointTaxation"]'),
    ...
    consultationSection: document.getElementById('consultationSection')
  };
  ...
  function updateRemainingRevenue() {
    ...
    if (difference > 0) {
      DOM.revenueInfoText.innerHTML = `<span style="color: var(--app-error)">Przekroczono przychód o: ${formatPLN(difference)}</span>`;
    } else {
      const remainingRevenue = totalRevenue - usedRevenue;
      DOM.revenueInfoText.textContent = `Przychód do rozdysponowania: ${formatPLN(remainingRevenue)}`;
    }
  }
  ...
  function calculate() {
    ...
    let revenue = parseFloat(rawRevenue.replace(/[^\d,.-]/g, "").replace(",", ".")) || 0;
    let costs = parseFloat(rawCosts.replace(/[^\d,.-]/g, "").replace(",", ".")) || 0;
    ...
    const healthContribLimit = 12900; // Maksymalna składka zdrowotna do odliczenia przy podatku liniowym 
    ...
    if (DOM.multipleRatesToggle.checked) {
      ...
      let ratesC17 = totalAllocatedRevenue > 300000 ? 12 * 1339.60 :
                    totalAllocatedRevenue > 60000 ? 12 * 744.22 :
                    12 * 446.53;
      F18 = ratesC17 * 0.5;
    } else {
      F18 = C17 * 0.5;
    ...
    let C6 = calculateTaxWithSpouse(income, false, 0, getCalculationValue("C18"));
    document.getElementById("C6").value = formatPLN(C6);
    if (document.querySelector('input[name="jointTaxation"]:checked').value === "yes") {
    ...
    if (document.querySelector('input[name="jointTaxation"]:checked').value === "yes") {
      const spouseIncome = parsePLN(document.getElementById("spouseIncome").value);
      const J11 = calculateSpouseFreeQuota0(spouseIncome);
      const K11 = calculateSpouseFreeQuota12(spouseIncome);
      const L11 = calculateSpouseFreeQuota32(spouseIncome);
      let F6_joint = income * ipBoxCoeff * 0.05 +
          Math.min(J3, 30000 + J11) * 0 +
          Math.min(Math.max(J3 - (30000 + J11), 0), 90000 + K11) * 0.12 +
          Math.min(Math.max(J3 - (30000 + J11) - (90000 + K11), 0), 880000 + L11) * 0.32 +
    ...
    function calculateG6(income, ipBoxCoeff, F17) {
      if (income - F17 > 77096.4) {
        ...
        const standardPart = standardIncome > 1000000 ? 
    ...
    function getAllocatedOrFullRateValue(rateId) {
      if (DOM.multipleRatesToggle.checked) {
        const rateInput = document.querySelector(`.rate-input[data-for="${rateId}"]`);
  ...
  function updateRatesTotal(C17) {
    ...
    const anyRateSelected = Array.from(DOM.ryczaltCheckboxes).some(checkbox => checkbox.checked);
    if (!anyRateSelected) {
      ratesTotalElement.classList.add("hidden");
      return;
    }
    ...
    const rateIds = ["C9", "D9", "E9", "F9", "G9", "C11", "D11", "E11", "F11", "G11"];
    let total = 0;
    rateIds.forEach((id) => {
  ...
  function handleCalculate() {
    ...
    if (isRevenueValid && isCostsValid && isIpBoxValid) {
      ...
      DOM.consultationSection.classList.remove("hidden");
      DOM.consultationSection.classList.add("show");
    }
    ...
    const anyChecked = Array.from(DOM.ryczaltCheckboxes).some((cb) => cb.checked);
    DOM.ryczaltMessage.style.display = anyChecked ? "none" : "block";

    const jointTaxationSelected = document.querySelector('input[name="jointTaxation"]:checked').value;
  ...
  DOM.ipBoxCoeffInput.addEventListener("input", (e) => {
    if (!e.target.hasAttribute("readonly") && validateIpBoxCoeff(e.target.value) && !DOM.resultsSection.classList.contains("hidden")) {
      calculate();
    }
  });
  ...
  DOM.jointTaxationRadios.forEach((radio) => {
    radio.addEventListener("change", (e) => {
      ...
      const jointTaxationCards = document.querySelectorAll(".joint-taxation-card");
  ...
  DOM.spouseIncomeInput.addEventListener("input", (e) => {
    if (!DOM.spouseIncomeCard.classList.contains("inactive")) {
      ...
      if (!DOM.resultsSection.classList.contains("hidden") && isValid) calculate();
    }
  });
  ...
  document.getElementById("multipleRatesToggle").addEventListener("change", function (e) {
  ...
  function getFormattedValues() {
    ...
    const anyRyczaltVisible = Array.from(DOM.ryczaltCheckboxes).some(checkbox => checkbox.checked);
    if (anyRyczaltVisible) {
      ...
      ryczaltRates.forEach(rate => {
          ...
          if (element && element.closest('.input-group').style.display !== 'none') {
