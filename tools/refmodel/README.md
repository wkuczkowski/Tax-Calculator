...
- **`FAMILY_OPTIONS` (domyślnie = przepis, alternatywy w `expectedFamily.json` → `alt`):** `capIncludesUndeductedHealth: true` (nieodliczona zdrowotna ryczałtowca/liniowca w limicie zwrotu – dosłownie ust. 9 pkt 2; aplikacja od a0a67be tak samo, RD6); `linearLinMethodCapSocial: 'undeducted'` (składki społeczne ponad dochód liniowy w limicie zwrotu; aplikacja od a0a67be tak samo, RD6); `singleParentLimitLinRycz: 'cautious'` (56 000 dla samotnego rodzica na liniowym/ryczałcie, SPEC); `estimateContribWhenZeroIncome: false`. Nadpisanie: `computeAll(inp, { family: { … } })`, w fuzzie zmienna `FAMILY_OPTS`.
...
- `KNOWN_DIFFS_FAMILY` w `compare.mjs` – dziś pusta lista. Wcześniejsze różnice w limicie zwrotu (nieodliczona zdrowotna, składki ponad dochód liniowy; FC-100, FC-103, FC-106, FC-109, FC-110, FE-151, FE-155, FH-186, FI-195, FI-205, FN-245) zniknęły po poprawce aplikacji a0a67be (wykładnia literalna art. 27f ust. 9, RD6).
